...
//     }
//   };
export const updatePatient = async (id: string, updatedData: Partial<CreatePatient>, accessToken: string): Promise<CreatePatient> => {
    try {
      const response = await axios.patch(`${API_BASE_URL}/patients/${id}`, updatedData, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  export const createPatient = async (patientData: CreatePatient, accessToken: string): Promise<CreatePatient> => {
    try {
      ...
      return response.data; // ✅ Return full patient data
    } catch (error: any) {
