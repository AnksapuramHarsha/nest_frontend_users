...
const PatientList: React.FC<PatientListProps> = ({ accessToken }) => {
    ...
    const handleDelete = async (upid: string) => {
        ...
        const reponse=await deletePatient(upid, accessToken);
        console.log("Delete Patient response:", reponse);
        if (reponse) {
            console.log("Deleted Patient:", reponse);
            fetchPatients(); // Refresh the patient list after deletion
        } else {
            console.error("Failed to delete patient:", reponse);
        }
    }
