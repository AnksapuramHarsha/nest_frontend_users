...
// import { fetchPatientsByNetwork, deletePatient } from "../apis/patientApi";
...
const PatientList: React.FC<PatientListProps> = ({ networkId }) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // ✅ Fetch updated data from backend instead of updating state manually
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.error(error.message || "Failed to delete patient.", { position: "top-right", autoClose: 3000 });
    }
  };
          ...
          <table className="min-w-full border border-gray-300">
            ...
            <tbody>
              {patients.map((patient) => (
                <tr key={patient.id} className="hover:bg-gray-50">
                  <td className="border border-gray-300 px-4 py-2">{patient.id}</td>
                  <td className="border border-gray-300 px-4 py-2">{patient.nameGiven} {patient.nameFamily}</td>
                  <td className="border border-gray-300 px-4 py-2 flex space-x-2">
                    <button onClick={() => handleEdit(patient)} className="px-3 py-1 bg-green-500 text-white rounded">Edit</button>
                    <button onClick={() => handleDelete(patient.id)} className="px-3 py-1 bg-red-500 text-white rounded">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
