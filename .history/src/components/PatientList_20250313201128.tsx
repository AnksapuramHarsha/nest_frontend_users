...
const PatientList: React.FC<PatientListProps> = ({ networkId }) => {
    ...
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);
    const [selectedPatient, setSelectedPatient] = useState<Crea | null>(null);
    const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
    const [isEditModalOpen, setIsEditModalOpen] = useState<boolean>(false);
    const [editFormData, setEditFormData] = useState<Partial<Patient>>({});
    const [searchTerm, setSearchTerm] = useState<string>("");
    ...
    const handleView = (id: string) => {
        const viewPatient = patients.find((p) => p.upid === id);
        if (patient) {
            setSelectedPatient();
            setIsModalOpen(true);
        } else {
    ...
    return (
        <div className="p-6 bg-white shadow-lg rounded-lg">
            <h2 className="text-2xl font-semibold text-gray-700 mb-5 text-center">Patient List</h2>
            <div className="flex justify-between mb-4">
                <input
                    type="text"
                    placeholder="Search by Name, ABHA, MRN, or Phone..."
